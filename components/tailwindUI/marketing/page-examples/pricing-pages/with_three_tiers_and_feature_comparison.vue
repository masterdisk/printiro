  ...
  <div class="bg-gray-50">
    <header class="bg-indigo-600">
      <nav class="mx-auto max-w-7xl px-6 lg:px-8" aria-label="Top">
        ...
        <div class="mt-4 flex flex-wrap justify-center gap-x-6 lg:hidden">
          <a v-for="item in navigation" :key="item.name" :href="item.href" class="text-base font-medium text-white hover:text-indigo-50">{{ item.name }}</a>
        </div>
      </nav>
    </header>
        ...
        <div class="relative bg-indigo-600">
          ...
          <div class="relative mt-12 flex justify-center sm:mt-16">
            <div class="flex rounded-lg bg-indigo-700 p-0.5">
              <button type="button" class="relative whitespace-nowrap rounded-md border-indigo-700 bg-white py-2 px-6 text-sm font-medium text-indigo-700 shadow-sm hover:bg-indigo-50 focus:z-10 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-indigo-700">
                Monthly billing
              </button>
              <button type="button" class="relative ml-0.5 whitespace-nowrap rounded-md border border-transparent py-2 px-6 text-sm font-medium text-indigo-200 hover:bg-indigo-800 focus:z-10 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-indigo-700">
                Yearly billing
              </button>
            </div>
          </div>
          ...
          <div class="relative mx-auto mt-8 max-w-2xl px-6 pb-8 sm:mt-12 lg:max-w-7xl lg:px-8 lg:pb-0">
            ...
            <div class="relative space-y-6 lg:grid lg:grid-cols-3 lg:space-y-0">
              <div v-for="plan in plans" :key="plan.title" class="pt-6 px-6 pb-3 rounded-lg lg:px-8 lg:pt-12" :class="[plan.featured ? 'bg-white ring-2 ring-indigo-700 shadow-md' : 'bg-indigo-700 lg:bg-transparent']">
                <div>
                  <h3 class="text-base font-semibold" :class="[plan.featured ? 'text-indigo-600' : 'text-white']">
                    {{ plan.title }}
                  </h3>
                  <div class="flex flex-col items-start sm:flex-row sm:items-center sm:justify-between lg:flex-col lg:items-start">
                    <div class="mt-3 flex items-center">
                      <p class="text-4xl font-bold tracking-tight" :class="[plan.featured ? 'text-indigo-600' : 'text-white']">
                        ${{ plan.priceMonthly }}
                      </p>
                      <div class="ml-4">
                        <p class="text-sm" :class="[plan.featured ? 'text-gray-700' : 'text-white']">
                          USD / mo
                        </p>
                        <p class="text-sm" :class="[plan.featured ? 'text-gray-500' : 'text-indigo-200']">
                          Billed yearly (${{ plan.priceYearly }})
                        </p>
                      </div>
                    </div>
                    <a href="#" class="mt-6 w-full inline-block py-2 px-8 border border-transparent rounded-md shadow-sm text-center text-sm font-medium sm:mt-0 sm:w-auto lg:mt-6 lg:w-full" :class="[plan.featured ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-white text-indigo-600 hover:bg-indigo-50']">Buy {{ plan.title }}</a>
                  </div>
                </div>
                <h4 class="sr-only">
                  Features
                </h4>
                <ul role="list" class="mt-7 border-t divide-y lg:border-t-0" :class="[plan.featured ? 'border-gray-200 divide-gray-200' : 'border-indigo-500 divide-indigo-500 divide-opacity-75']">
                  <li v-for="mainFeature in plan.mainFeatures" :key="mainFeature.id" class="flex items-center py-3">
                    <CheckIcon class="w-5 h-5 flex-shrink-0" :class="[plan.featured ? 'text-indigo-500' : 'text-indigo-200']" aria-hidden="true" />
                    <span class="ml-4 text-sm font-medium" :class="[plan.featured ? 'text-gray-600' : 'text-white']">{{ mainFeature.value }}</span>
                  </li>
                </ul>
              </div>
            </div>
        ...
        <section aria-labelledby="mobile-comparison-heading" class="lg:hidden">
          ...
          <div class="mx-auto mt-16 max-w-2xl space-y-16 px-6">
            <div v-for="(plan, planIndex) in plans" :key="plan.title" class="border-t border-gray-200">
              <div class="-mt-px pt-6 border-t-2 sm:w-1/2" :class="[plan.featured ? 'border-indigo-600' : 'border-transparent']">
                <h3 class="text-sm font-bold" :class="[plan.featured ? 'text-indigo-600' : 'text-gray-900']">
                  {{ plan.title }}
                </h3>
              ...
              <div class="relative mt-6">
                ...
                <div aria-hidden="true" class="pointer-events-none absolute inset-0 hidden sm:block">
                  <div class="absolute right-0 w-1/2 h-full bg-white rounded-lg" :class="[plan.featured ? 'shadow-md' : 'shadow']" />
                </div>

                <div class="relative py-3 px-4 bg-white rounded-lg sm:p-0 sm:bg-transparent sm:rounded-none sm:ring-0 sm:shadow-none" :class="[plan.featured ? 'ring-2 ring-indigo-600 shadow-md' : 'ring-1 ring-black ring-opacity-5 shadow']">
                  <dl class="divide-y divide-gray-200">
                    <div v-for="feature in features" :key="feature.title" class="flex items-center justify-between py-3 sm:grid sm:grid-cols-2">
                      ...
                      <dd class="flex items-center justify-end sm:justify-center sm:px-4">
                        <span v-if="typeof feature.tiers[planIndex].value === 'string'" class="text-sm font-medium" :class="[feature.tiers[planIndex].featured ? 'text-indigo-600' : 'text-gray-900']">{{ feature.tiers[planIndex].value }}</span>
                        <template v-else>
                ...
                <!-- Fake card border -->
                <div aria-hidden="true" class="pointer-events-none absolute inset-0 hidden sm:block">
                  <div class="absolute right-0 w-1/2 h-full rounded-lg" :class="[plan.featured ? 'ring-2 ring-indigo-600' : 'ring-1 ring-black ring-opacity-5']" />
                </div>
              </div>
        ...
        <section aria-labelledby="comparison-heading" class="hidden lg:block">
          ...
          <div class="mx-auto mt-24 max-w-7xl px-8">
            <div class="flex w-full items-stretch border-t border-gray-200">
              ...
              <div v-for="(plan, index) in plans" :key="plan.title" aria-hidden="true" class="-mt-px pl-4 w-1/4" :class="[index === plans.length - 1 ? '' : 'pr-4']">
                <div class="py-6 border-t-2" :class="[plan.featured ? 'border-indigo-600' : 'border-transparent']">
                  <p class="text-sm font-bold" :class="[plan.featured ? 'text-indigo-600' : 'text-gray-900']">
                    {{ plan.title }}
                  </p>
                  <p class="mt-2 text-sm text-gray-500">
                    {{ plan.description }}
              ...
              <table class="relative w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                  <tr v-for="(feature, featureIndex) in features" :key="featureIndex">
                    ...
                    <td v-for="(tier, index) in feature.tiers" :key="tier.title" class="relative w-1/4 py-0 text-center" :class="[index === feature.tiers.length - 1 ? 'pl-4' : 'px-4']">
                      <span class="relative h-full w-full py-3">
                        <span v-if="typeof tier.value === 'string'" class="text-sm font-medium" :class="[tier.featured ? 'text-indigo-600' : 'text-gray-900']">{{ tier.value }}</span>
                        <template v-else>
              ...
              <table class="relative w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                  <tr v-for="perk in perks" :key="perk.title">
                    ...
                    <td v-for="(tier, index) in perk.tiers" :key="tier.title" class="relative w-1/4 py-0 text-center" :class="[index === perk.tiers.length - 1 ? 'pl-4' : 'px-4']">
                      <span class="relative h-full w-full py-3">
                        <CheckIcon v-if="tier.value === true" class="mx-auto h-5 w-5 text-indigo-600" aria-hidden="true" />
                        <XMarkIcon v-else class="mx-auto h-5 w-5 text-gray-400" aria-hidden="true" />
                        <span class="sr-only">{{ tier.value === true ? 'Yes' : 'No' }}</span>
      ...
      <section aria-labelledby="faq-heading" class="bg-gray-900">
        <div class="mx-auto max-w-2xl py-16 px-6 sm:py-24 lg:max-w-7xl lg:px-8">
          <div class="max-w-2xl lg:mx-auto lg:text-center">
            <h2 id="faq-heading" class="text-3xl font-bold tracking-tight text-white sm:text-4xl">
