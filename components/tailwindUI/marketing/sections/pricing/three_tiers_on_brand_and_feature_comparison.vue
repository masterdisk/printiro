          ...
          <div v-for="plan in plans" :key="plan.title" class="pt-6 px-6 pb-3 rounded-lg lg:px-8 lg:pt-12" :class="[plan.featured ? 'bg-white ring-2 ring-indigo-700 shadow-md' : 'bg-indigo-700 lg:bg-transparent']">
            ...
            <ul role="list" class="mt-7 border-t divide-y lg:border-t-0" :class="[plan.featured ? 'border-gray-200 divide-gray-200' : 'border-indigo-500 divide-indigo-500 divide-opacity-75']">
              <li v-for="mainFeature in plan.mainFeatures" :key="mainFeature.id" class="flex items-center py-3">
                ...
                <span class="ml-3 text-sm font-medium" :class="[plan.featured ? 'text-gray-600' : 'text-white']">{{ mainFeature.value }}</span>
              </li>
            </ul>
          </div>
        </div>
              ...
              <dl class="divide-y divide-gray-200">
                <div v-for="feature in features" :key="feature.title" class="flex items-center justify-between py-3 sm:grid sm:grid-cols-2">
                  ...
                  <dd class="flex items-center justify-end sm:justify-center sm:px-4">
                    <span v-if="typeof feature.tiers[mobilePlanIndex].value === 'string'" class="text-sm font-medium" :class="[feature.tiers[mobilePlanIndex].featured ? 'text-indigo-600' : 'text-gray-900']">{{ feature.tiers[mobilePlanIndex].value }}</span>
                    <template v-else>
    ...
    <section aria-labelledby="comparison-heading" class="hidden lg:block">
      ...
      <div class="mx-auto max-w-7xl py-24 px-8">
        <div class="flex w-full items-stretch border-t border-gray-200">
          ...
          <div v-for="(plan, planIdx) in plans" :key="plan.title" aria-hidden="true" class="-mt-px pl-4 w-1/4" :class="[planIdx === plans.length - 1 ? '' : 'pr-4']">
          ...
          <table class="relative w-full">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="feature in features" :key="feature.title">
                ...
                <td v-for="(tier, tierIdx) in feature.tiers" :key="tier.title" class="relative w-1/4 py-0 text-center" :class="[tierIdx === feature.tiers.length - 1 ? 'pl-4' : 'px-4']">
          ...
          <table class="relative w-full">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="perk in perks" :key="perk.title">
                ...
                <td v-for="(tier, tierIdx) in perk.tiers" :key="tier.title" class="relative w-1/4 py-0 text-center" :class="[tierIdx === perk.tiers.length - 1 ? 'pl-4' : 'px-4']">
                  <span class="relative h-full w-full py-3">
                    <CheckIcon v-if="tier.value === true" class="mx-auto h-5 w-5 text-indigo-600" aria-hidden="true" />
                    <XMarkIcon v-else class="mx-auto h-5 w-5 text-gray-400" aria-hidden="true" />
                    <span class="sr-only">{{ tier.value === true ? 'Yes' : 'No' }}</span>
