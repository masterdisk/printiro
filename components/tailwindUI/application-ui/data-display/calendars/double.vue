...
<template>
  <div>
    <div class="relative grid grid-cols-1 gap-x-14 md:grid-cols-2">
      ...
      <section v-for="(month, monthIdx) in months" :key="monthIdx" class="text-center" :class="[monthIdx === months.length - 1 && 'hidden md:block']">
        ...
        <div class="isolate mt-2 grid grid-cols-7 gap-px rounded-lg bg-gray-200 text-sm shadow ring-1 ring-gray-200">
          <button v-for="(day, dayIdx) in month.days" :key="day.date" type="button" class="relative py-1.5 hover:bg-gray-100 focus:z-10" :class="[day.isCurrentMonth ? 'bg-white text-gray-900' : 'bg-gray-50 text-gray-400', dayIdx === 0 && 'rounded-tl-lg', dayIdx === 6 && 'rounded-tr-lg', dayIdx === month.days.length - 7 && 'rounded-bl-lg', dayIdx === month.days.length - 1 && 'rounded-br-lg']">
            <time :datetime="day.date" class="mx-auto flex h-7 w-7 items-center justify-center rounded-full" :class="[day.isToday && 'bg-indigo-600 font-semibold text-white']">{{ day.date.split('-').pop().replace(/^0/, '') }}</time>
          </button>
        </div>
      </section>
    </div>
