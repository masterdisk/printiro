    ...
    <div class="flex flex-1 flex-col overflow-y-auto pt-5 pb-4">
      ...
      <nav class="mt-5 flex-1 space-y-1 bg-gray-800 px-2" aria-label="Sidebar">
        <a v-for="item in navigation" :key="item.name" :href="item.href" class="group flex items-center px-2 py-2 text-sm font-medium rounded-md" :class="[item.current ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white']">
          ...
          <span v-if="item.count" class="ml-3 inline-block py-0.5 px-3 text-xs font-medium rounded-full" :class="[item.current ? 'bg-gray-800' : 'bg-gray-900 group-hover:bg-gray-800']">{{ item.count }}</span>
        </a>
      </nav>
    </div>
