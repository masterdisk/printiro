  ...
  <div class="flex min-h-0 flex-1 flex-col bg-indigo-700">
    <div class="flex flex-1 flex-col overflow-y-auto pt-5 pb-4">
      ...
      <nav class="mt-5 flex-1 space-y-1 px-2" aria-label="Sidebar">
        <a v-for="item in navigation" :key="item.name" :href="item.href" class="group flex items-center px-2 py-2 text-sm font-medium rounded-md" :class="[item.current ? 'bg-indigo-800 text-white' : 'text-indigo-100 hover:bg-indigo-600 hover:bg-opacity-75']">
          <component :is="item.icon" class="mr-3 h-6 w-6 flex-shrink-0 text-indigo-300" aria-hidden="true" />
          <span class="flex-1">{{ item.name }}</span>
          <span v-if="item.count" class="ml-3 inline-block py-0.5 px-3 text-xs font-medium rounded-full" :class="[item.current ? 'bg-indigo-600' : 'bg-indigo-800']">{{ item.count }}</span>
        </a>
      </nav>
    </div>
