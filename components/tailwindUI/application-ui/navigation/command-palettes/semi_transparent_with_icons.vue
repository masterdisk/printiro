                ...
                <li class="p-2">
                  ...
                  <ul class="text-sm text-gray-700">
                    <ComboboxOption v-for="project in query === '' ? recent : filteredProjects" :key="project.id" v-slot="{ active }" :value="project" as="template">
                      <li class="flex cursor-default select-none items-center rounded-md px-3 py-2" :class="[active && 'bg-gray-900 bg-opacity-5 text-gray-900']">
                        <FolderIcon class="h-6 w-6 flex-none text-gray-900 text-opacity-40" :class="[active && 'text-opacity-100']" aria-hidden="true" />
                ...
                <li v-if="query === ''" class="p-2">
                  ...
                  <ul class="text-sm text-gray-700">
                    <ComboboxOption v-for="action in quickActions" :key="action.shortcut" v-slot="{ active }" :value="action" as="template">
                      <li class="flex cursor-default select-none items-center rounded-md px-3 py-2" :class="[active && 'bg-gray-900 bg-opacity-5 text-gray-900']">
                        <component :is="action.icon" class="h-6 w-6 flex-none text-gray-900 text-opacity-40" :class="[active && 'text-opacity-100']" aria-hidden="true" />
