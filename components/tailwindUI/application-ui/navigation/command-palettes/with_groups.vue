              ...
              <ComboboxOptions v-if="filteredItems.length > 0" static class="max-h-80 scroll-pt-11 scroll-pb-2 space-y-2 overflow-y-auto pb-2">
                <li v-for="[category, items] in Object.entries(groups)" :key="category">
                  ...
                  <ul class="mt-2 text-sm text-gray-800">
                    <ComboboxOption v-for="item in items" :key="item.id" v-slot="{ active }" :value="item" as="template">
                      <li class="cursor-default select-none px-4 py-2" :class="[active && 'bg-indigo-600 text-white']">
                        {{ item.name }}
                      </li>
                    </ComboboxOption>
                  </ul>
