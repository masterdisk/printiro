                ...
                <li class="p-2">
                  ...
                  <ul class="text-sm text-gray-400">
                    <ComboboxOption v-for="project in query === '' ? recent : filteredProjects" :key="project.id" v-slot="{ active }" :value="project" as="template">
                      <li class="flex cursor-default select-none items-center rounded-md px-3 py-2" :class="[active && 'bg-gray-800 text-white']">
                        <FolderIcon class="h-6 w-6 flex-none" :class="[active ? 'text-white' : 'text-gray-500']" aria-hidden="true" />
                ...
                <li v-if="query === ''" class="p-2">
                  ...
                  <ul class="text-sm text-gray-400">
                    <ComboboxOption v-for="action in quickActions" :key="action.shortcut" v-slot="{ active }" :value="action" as="template">
                      <li class="flex cursor-default select-none items-center rounded-md px-3 py-2" :class="[active && 'bg-gray-800 text-white']">
                        <component :is="action.icon" class="h-6 w-6 flex-none" :class="[active ? 'text-white' : 'text-gray-500']" aria-hidden="true" />
