          ...
          <DialogPanel class="mx-auto max-w-3xl transform divide-y divide-gray-100 overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black ring-opacity-5 transition-all">
            <Combobox v-slot="{ activeOption }" @update:modelValue="onSelect">
              ...
              <ComboboxOptions v-if="query === '' || filteredPeople.length > 0" class="flex divide-x divide-gray-100" as="div" static hold>
                <div class="max-h-96 min-w-0 flex-auto scroll-py-4 overflow-y-auto px-6 py-4" :class="[activeOption && 'sm:h-96']">
                  <h2 v-if="query === ''" class="mt-2 mb-4 text-xs font-semibold text-gray-500">
                    Recent searches
                  </h2>
                  <div hold class="-mx-2 text-sm text-gray-700">
                    <ComboboxOption v-for="person in query === '' ? recent : filteredPeople" :key="person.id" v-slot="{ active }" :value="person" as="template">
                      <div class="group flex cursor-default select-none items-center rounded-md p-2" :class="[active && 'bg-gray-100 text-gray-900']">
