          ...
          <DialogPanel class="mx-auto max-w-2xl transform divide-y divide-gray-100 overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black ring-opacity-5 transition-all">
            <Combobox @update:modelValue="onSelect">
              ...
              <ComboboxOptions v-if="query === '' || filteredProjects.length > 0" static class="max-h-80 scroll-py-2 divide-y divide-gray-100 overflow-y-auto">
                <li class="p-2">
                  <h2 v-if="query === ''" class="mt-4 mb-2 px-3 text-xs font-semibold text-gray-500">
                    Recent searches
                  </h2>
                  <ul class="text-sm text-gray-700">
                    <ComboboxOption v-for="project in query === '' ? recent : filteredProjects" :key="project.id" v-slot="{ active }" :value="project" as="template">
                      <li class="flex cursor-default select-none items-center rounded-md px-3 py-2" :class="[active && 'bg-indigo-600 text-white']">
                ...
                <li v-if="query === ''" class="p-2">
                  ...
                  <ul class="text-sm text-gray-700">
                    <ComboboxOption v-for="action in quickActions" :key="action.shortcut" v-slot="{ active }" :value="action" as="template">
                      <li class="flex cursor-default select-none items-center rounded-md px-3 py-2" :class="[active && 'bg-indigo-600 text-white']">
                        <component :is="action.icon" class="h-6 w-6 flex-none" :class="[active ? 'text-white' : 'text-gray-400']" aria-hidden="true" />
                        <span class="ml-3 flex-auto truncate">{{ action.name }}</span>
                        <span class="ml-3 flex-none text-xs font-semibold" :class="[active ? 'text-indigo-100' : 'text-gray-400']">
