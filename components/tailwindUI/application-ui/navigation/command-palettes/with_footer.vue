            ...
            <Combobox @update:modelValue="onSelect">
              ...
              <ComboboxOptions v-if="filteredProjects.length > 0 || filteredUsers.length > 0" static class="max-h-80 scroll-py-10 scroll-pb-2 space-y-4 overflow-y-auto p-4 pb-2">
                <li v-if="filteredProjects.length > 0">
                  ...
                  <ul class="-mx-4 mt-2 text-sm text-gray-700">
                    <ComboboxOption v-for="project in filteredProjects" :key="project.id" v-slot="{ active }" :value="project" as="template">
                      <li class="flex cursor-default select-none items-center px-4 py-2" :class="[active && 'bg-indigo-600 text-white']">
                        <FolderIcon class="h-6 w-6 flex-none" :class="[active ? 'text-white' : 'text-gray-400']" aria-hidden="true" />
                ...
                <li v-if="filteredUsers.length > 0">
                  ...
                  <ul class="-mx-4 mt-2 text-sm text-gray-700">
                    <ComboboxOption v-for="user in filteredUsers" :key="user.id" v-slot="{ active }" :value="user" as="template">
                      <li class="flex cursor-default select-none items-center px-4 py-2" :class="[active && 'bg-indigo-600 text-white']">
              ...
              <div v-if="rawQuery === '?'" class="py-14 px-6 text-center text-sm sm:px-14">
                ...
                <p class="mt-2 text-gray-500">
                  Use this tool to quickly search for users and projects across our entire platform. You can also use the search modifiers found in the footer below to limit the results to just users or projects.
                </p>
              </div>
              ...
              <div class="flex flex-wrap items-center bg-gray-50 py-2.5 px-4 text-xs text-gray-700">
                Type
                <kbd class="mx-1 flex h-5 w-5 items-center justify-center rounded border bg-white font-semibold sm:mx-2" :class="[rawQuery.startsWith('#') ? 'border-indigo-600 text-indigo-600' : 'border-gray-400 text-gray-900']">#</kbd>
                <span class="sm:hidden">for projects,</span>
                <span class="hidden sm:inline">to access projects,</span>
                <kbd class="mx-1 flex h-5 w-5 items-center justify-center rounded border bg-white font-semibold sm:mx-2" :class="[rawQuery.startsWith('>') ? 'border-indigo-600 text-indigo-600' : 'border-gray-400 text-gray-900']">&gt;</kbd>
                for users, and
                <kbd class="mx-1 flex h-5 w-5 items-center justify-center rounded border bg-white font-semibold sm:mx-2" :class="[rawQuery === '?' ? 'border-indigo-600 text-indigo-600' : 'border-gray-400 text-gray-900']">?</kbd>
                for help.
              </div>
            </Combobox>
          </DialogPanel>
