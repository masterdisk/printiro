...
<template>
  ...
  <div class="min-h-full">
    ...
    <Popover v-slot="{ open }" as="template">
      <header class="bg-white shadow-sm lg:static lg:overflow-y-visible" :class="[open ? 'fixed inset-0 z-40 overflow-y-auto' : '']">
        ...
        <PopoverPanel as="nav" class="lg:hidden" aria-label="Global">
          <div class="mx-auto max-w-3xl space-y-1 px-2 pt-2 pb-3 sm:px-4">
            <a v-for="item in navigation" :key="item.name" :href="item.href" :aria-current="item.current ? 'page' : undefined" class="block rounded-md py-2 px-3 text-base font-medium" :class="[item.current ? 'bg-gray-100 text-gray-900' : 'hover:bg-gray-50']">{{ item.name }}</a>
          </div>
        ...
        <div class="hidden lg:col-span-3 lg:block xl:col-span-2">
          <nav aria-label="Sidebar" class="sticky top-4 divide-y divide-gray-300">
            <div class="space-y-1 pb-8">
              <a v-for="item in navigation" :key="item.name" :href="item.href" class="group flex items-center px-3 py-2 text-sm font-medium rounded-md" :class="[item.current ? 'bg-gray-200 text-gray-900' : 'text-gray-700 hover:bg-gray-50']" :aria-current="item.current ? 'page' : undefined">
            ...
            <div class="pt-10">
              <p id="communities-headline" class="px-3 text-sm font-medium text-gray-500">
                Communities
              </p>
          ...
          <div class="px-4 sm:px-0">
            ...
            <div class="hidden sm:block">
              <nav class="isolate flex divide-x divide-gray-200 rounded-lg shadow" aria-label="Tabs">
                <a v-for="(tab, tabIdx) in tabs" :key="tab.name" :href="tab.href" :aria-current="tab.current ? 'page' : undefined" class="group relative min-w-0 flex-1 overflow-hidden bg-white py-4 px-6 text-sm font-medium text-center hover:bg-gray-50 focus:z-10" :class="[tab.current ? 'text-gray-900' : 'text-gray-500 hover:text-gray-700', tabIdx === 0 ? 'rounded-l-lg' : '', tabIdx === tabs.length - 1 ? 'rounded-r-lg' : '']">
                  <span>{{ tab.name }}</span>
                  <span aria-hidden="true" class="absolute inset-x-0 bottom-0 h-0.5" :class="[tab.current ? 'bg-rose-500' : 'bg-transparent']" />
                </a>
              </nav>
            </div>
          </div>
                            ...
                            <MenuItems class="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                              <div class="py-1">
                                <MenuItem v-slot="{ active }">
                                  <a href="#" class="flex px-4 py-2 text-sm" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700']">
