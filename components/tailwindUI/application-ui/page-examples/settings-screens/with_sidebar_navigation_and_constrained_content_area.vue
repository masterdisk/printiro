            ...
            <DialogPanel class="relative flex w-full max-w-xs flex-1 flex-col bg-white pt-5 pb-4">
              ...
              <div class="mt-5 h-0 flex-1 overflow-y-auto">
                <nav class="flex h-full flex-col">
                  <div class="space-y-1">
                    <a v-for="item in navigation" :key="item.name" :href="item.href" class="group border-l-4 py-2 px-3 flex items-center text-base font-medium" :class="[item.current ? 'bg-purple-50 border-purple-600 text-purple-600' : 'border-transparent text-gray-600 hover:bg-gray-50 hover:text-gray-900']" :aria-current="item.current ? 'page' : undefined">
                      <component :is="item.icon" class="mr-4 flex-shrink-0 h-6 w-6" :class="[item.current ? 'text-purple-500' : 'text-gray-400 group-hover:text-gray-500']" aria-hidden="true" />
                      {{ item.name }}
                    </a>
                  </div>
    ...
    <div class="hidden md:fixed md:inset-y-0 md:flex md:w-64 md:flex-col">
      ...
      <nav class="flex flex-grow flex-col overflow-y-auto border-r border-gray-200 bg-gray-50 pt-5 pb-4">
        ...
        <div class="mt-5 flex-grow">
          <div class="space-y-1">
            <a v-for="item in navigation" :key="item.name" :href="item.href" class="group border-l-4 py-2 px-3 flex items-center text-sm font-medium" :class="[item.current ? 'bg-purple-50 border-purple-600 text-purple-600' : 'border-transparent text-gray-600 hover:text-gray-900 hover:bg-gray-50']">
              <component :is="item.icon" class="mr-3 flex-shrink-0 h-6 w-6" :class="[item.current ? 'text-purple-500' : 'text-gray-400 group-hover:text-gray-500']" aria-hidden="true" />
              {{ item.name }}
            </a>
          </div>
        </div>
            ...
            <div class="flex flex-1">
              <form class="flex w-full md:ml-0" action="#" method="GET">
                ...
                <div class="relative w-full text-gray-400 focus-within:text-gray-600">
                  ...
                  <input id="mobile-search-field" name="mobile-search-field" class="h-full w-full border-transparent py-2 pl-8 pr-3 text-base text-gray-900 placeholder-gray-500 focus:border-transparent focus:placeholder-gray-400 focus:outline-none focus:ring-0 sm:hidden" placeholder="Search" type="search">
                  <input id="desktop-search-field" name="desktop-search-field" class="hidden h-full w-full border-transparent py-2 pl-8 pr-3 text-sm text-gray-900 placeholder-gray-500 focus:border-transparent focus:placeholder-gray-400 focus:outline-none focus:ring-0 sm:block" placeholder="Search jobs, applicants, and more" type="search">
                </div>
              </form>
            </div>
                  ...
                  <div class="hidden lg:block">
                    <div class="border-b border-gray-200">
                      <nav class="-mb-px flex space-x-8">
                        <a v-for="tab in tabs" :key="tab.name" :href="tab.href" class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm" :class="[tab.current ? 'border-purple-500 text-purple-600' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700']">{{ tab.name }}</a>
                      </nav>
                    </div>
                  </div>
                    ...
                    <div class="mt-6">
                      <dl class="divide-y divide-gray-200">
                        ...
                        <SwitchGroup as="div" class="py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:py-5 sm:pt-5">
                          ...
                          <dd class="mt-1 flex text-sm text-gray-900 sm:col-span-2 sm:mt-0">
                            <Switch v-model="automaticTimezoneEnabled" class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 sm:ml-auto" :class="[automaticTimezoneEnabled ? 'bg-purple-600' : 'bg-gray-200']">
                              <span aria-hidden="true" class="inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out" :class="[automaticTimezoneEnabled ? 'translate-x-5' : 'translate-x-0']" />
                            </Switch>
                          </dd>
                        </SwitchGroup>
                        <SwitchGroup as="div" class="py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:border-b sm:border-gray-200 sm:py-5">
                          ...
                          <dd class="mt-1 flex text-sm text-gray-900 sm:col-span-2 sm:mt-0">
                            <Switch v-model="autoUpdateApplicantDataEnabled" class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 sm:ml-auto" :class="[autoUpdateApplicantDataEnabled ? 'bg-purple-600' : 'bg-gray-200']">
                              <span aria-hidden="true" class="inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out" :class="[autoUpdateApplicantDataEnabled ? 'translate-x-5' : 'translate-x-0']" />
                            </Switch>
                          </dd>
                        </SwitchGroup>
                      </dl>
