                  ...
                  <MenuItems class="absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div class="py-1">
                      <MenuItem v-for="child in item.children" :key="child.name" v-slot="{ active }">
                        <a :href="child.href" class="block px-4 py-2 text-sm text-gray-700" :class="[active ? 'bg-gray-100' : '']">{{ child.name }}</a>
                      </MenuItem>
                    </div>
                  </MenuItems>
                </transition>
    ...
    <div class="flex min-h-0 flex-1 overflow-hidden">
      ...
      <nav aria-label="Sidebar" class="hidden lg:block lg:flex-shrink-0 lg:overflow-y-auto lg:bg-gray-800">
        <div class="relative flex w-20 flex-col space-y-3 p-3">
          <a v-for="item in sidebarNavigation" :key="item.name" :href="item.href" class="flex-shrink-0 inline-flex items-center justify-center h-14 w-14 rounded-lg" :class="[item.current ? 'bg-gray-900 text-white' : 'text-gray-400 hover:bg-gray-700']">
                      ...
                      <Menu as="div" class="relative -ml-px block sm:shadow-sm lg:hidden">
                        ...
                        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                          <MenuItems class="absolute right-0 z-10 mt-2 w-36 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                            <div class="py-1">
                              <MenuItem v-slot="{ active }">
                                <a href="#" class="block px-4 py-2 text-sm sm:hidden" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700']">Note</a>
                              </MenuItem>
                              <MenuItem v-slot="{ active }">
                                <a href="#" class="block px-4 py-2 text-sm sm:hidden" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700']">Assign</a>
                              </MenuItem>
                              <MenuItem v-slot="{ active }">
