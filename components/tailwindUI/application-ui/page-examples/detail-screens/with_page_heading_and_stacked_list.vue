    ...
    <Disclosure v-slot="{ open }" as="nav" class="bg-gray-50">
          ...
          <div class="flex items-center">
            ...
            <div class="hidden lg:ml-10 lg:block">
              <div class="flex space-x-4">
                <a v-for="item in navigation" :key="item.name" :href="item.href" class="px-3 py-2 rounded-md text-sm font-medium text-gray-900" :class="[item.current ? 'bg-gray-100' : 'hover:text-gray-700']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</a>
              </div>
            </div>
          </div>
      ...
      <DisclosurePanel class="border-b border-gray-200 bg-gray-50 lg:hidden">
        <div class="space-y-1 px-2 pt-2 pb-3">
          <DisclosureButton v-for="item in navigation" :key="item.name" as="a" :href="item.href" class="block px-3 py-2 rounded-md font-medium text-gray-900" :class="[item.current ? 'bg-gray-100' : 'hover:bg-gray-100']" :aria-current="item.current ? 'page' : undefined">
            {{ item.name }}
          </DisclosureButton>
        </div>
        <div class="border-t border-gray-200 pt-4 pb-3">
          ...
          <div class="mt-3 space-y-1 px-2">
            <DisclosureButton v-for="item in userNavigation" :key="item.name" as="a" :href="item.href" class="block rounded-md py-2 px-3 text-base font-medium text-gray-900 hover:bg-gray-100">
              {{ item.name }}
            </DisclosureButton>
          </div>
        </div>
    ...
    <header class="bg-gray-50 py-8">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 xl:flex xl:items-center xl:justify-between">
        ...
        <div class="mt-5 flex xl:mt-0 xl:ml-4">
          ...
          <Listbox v-model="selected" as="div" class="sm:ml-3">
            ...
            <div class="relative">
              ...
              <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
                <ListboxOptions class="absolute left-0 z-10 mt-2 -mr-1 w-72 origin-top-right divide-y divide-gray-200 overflow-hidden rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:left-auto sm:right-0">
                  <ListboxOption v-for="option in publishingOptions" :key="option.name" v-slot="{ active, selected }" as="template" :value="option">
                    <li class="cursor-default select-none p-4 text-sm" :class="[active ? 'text-white bg-purple-500' : 'text-gray-900']">
                      <div class="flex flex-col">
                        ...
                        <p class="mt-2" :class="[active ? 'text-purple-200' : 'text-gray-500']">
                          {{ option.description }}
                        </p>
                      </div>
                    </li>
          ...
          <div class="hidden sm:block">
            <div class="border-b border-gray-200">
              <nav class="mt-2 -mb-px flex space-x-8" aria-label="Tabs">
                <a v-for="tab in tabs" :key="tab.name" :href="tab.href" class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm" :class="[tab.current ? 'border-purple-500 text-purple-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-200']">
                  {{ tab.name }}
                  <span v-if="tab.count" class="hidden ml-2 py-0.5 px-2.5 rounded-full text-xs font-medium md:inline-block" :class="[tab.current ? 'bg-purple-100 text-purple-600' : 'bg-gray-100 text-gray-900']">{{ tab.count }}</span>
                </a>
              </nav>
            </div>
          </div>
