    ...
    <div class="hidden w-28 overflow-y-auto bg-indigo-700 md:block">
      <div class="flex w-full flex-col items-center py-6">
        ...
        <div class="mt-6 w-full flex-1 space-y-1 px-2">
          <a v-for="item in navigation" :key="item.name" :href="item.href" class="group w-full p-3 rounded-md flex flex-col items-center text-xs font-medium" :class="[item.current ? 'bg-indigo-800 text-white' : 'text-indigo-100 hover:bg-indigo-800 hover:text-white']" :aria-current="item.current ? 'page' : undefined">
            <component :is="item.icon" class="h-6 w-6" :class="[item.current ? 'text-white' : 'text-indigo-300 group-hover:text-white']" aria-hidden="true" />
            ...
            <DialogPanel class="relative flex w-full max-w-xs flex-1 flex-col bg-indigo-700 pt-5 pb-4">
              ...
              <div class="mt-5 h-0 flex-1 overflow-y-auto px-2">
                <nav class="flex h-full flex-col">
                  <div class="space-y-1">
                    <a v-for="item in navigation" :key="item.name" :href="item.href" class="group py-2 px-3 rounded-md flex items-center text-sm font-medium" :class="[item.current ? 'bg-indigo-800 text-white' : 'text-indigo-100 hover:bg-indigo-800 hover:text-white']" :aria-current="item.current ? 'page' : undefined">
                      <component :is="item.icon" class="mr-3 h-6 w-6" :class="[item.current ? 'text-white' : 'text-indigo-300 group-hover:text-white']" aria-hidden="true" />
                      <span>{{ item.name }}</span>
                    </a>
                  </div>
                </nav>
            ...
            <div class="flex flex-1">
              <form class="flex w-full md:ml-0" action="#" method="GET">
                ...
                <div class="relative w-full text-gray-400 focus-within:text-gray-600">
                  ...
                  <input id="desktop-search-field" name="desktop-search-field" class="hidden h-full w-full border-transparent py-2 pl-8 pr-3 text-base text-gray-900 placeholder-gray-500 focus:border-transparent focus:placeholder-gray-400 focus:outline-none focus:ring-0 sm:block" placeholder="Search all files" type="search">
                </div>
              </form>
            </div>
              ...
              <div class="hidden sm:block">
                <div class="flex items-center border-b border-gray-200">
                  <nav class="-mb-px flex flex-1 space-x-6 xl:space-x-8" aria-label="Tabs">
                    <a v-for="tab in tabs" :key="tab.name" :href="tab.href" :aria-current="tab.current ? 'page' : undefined" class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm" :class="[tab.current ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300']">{{ tab.name }}</a>
                  </nav>
            ...
            <section class="mt-8 pb-16" aria-labelledby="gallery-heading">
              ...
              <ul role="list" class="grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
                <li v-for="file in files" :key="file.name" class="relative">
                  <div class="group block w-full aspect-w-10 aspect-h-7 rounded-lg bg-gray-100 overflow-hidden" :class="[file.current ? 'ring-2 ring-offset-2 ring-indigo-500' : 'focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-offset-gray-100 focus-within:ring-indigo-500']">
                    <img :src="file.source" alt="" class="object-cover pointer-events-none" :class="[file.current ? '' : 'group-hover:opacity-75']">
              ...
              <div class="mt-4 flex items-start justify-between">
                <div>
                  <h2 class="text-lg font-medium text-gray-900">
                    <span class="sr-only">Details for </span>{{ currentFile.name }}
                  </h2>
