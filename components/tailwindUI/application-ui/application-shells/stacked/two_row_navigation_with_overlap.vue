          ...
          <div class="grid grid-cols-3 items-center gap-8">
            <div class="col-span-2">
              <nav class="flex space-x-4">
                <a v-for="item in navigation" :key="item.name" :href="item.href" class="text-sm font-medium rounded-md bg-white bg-opacity-0 px-3 py-2 hover:bg-opacity-10" :class="[item.current ? 'text-white' : 'text-indigo-100']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</a>
              </nav>
            </div>
            <div>
