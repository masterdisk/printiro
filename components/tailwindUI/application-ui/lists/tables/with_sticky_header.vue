            ...
            <table class="min-w-full border-separate" style="border-spacing: 0">
              ...
              <tbody class="bg-white">
                <tr v-for="(person, personIdx) in people" :key="person.email">
                  <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6 lg:pl-8" :class="[personIdx !== people.length - 1 ? 'border-b border-gray-200' : '']">
                    {{ person.name }}
                  </td>
                  <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 hidden sm:table-cell" :class="[personIdx !== people.length - 1 ? 'border-b border-gray-200' : '']">
                    {{ person.title }}
                  </td>
                  <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 hidden lg:table-cell" :class="[personIdx !== people.length - 1 ? 'border-b border-gray-200' : '']">
                    {{ person.email }}
                  </td>
                  <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500" :class="[personIdx !== people.length - 1 ? 'border-b border-gray-200' : '']">
                    {{ person.role }}
                  </td>
                  <td class="relative whitespace-nowrap py-4 pr-4 pl-3 text-right text-sm font-medium sm:pr-6 lg:pr-8" :class="[personIdx !== people.length - 1 ? 'border-b border-gray-200' : '']">
