    ...
    <div class="relative mt-1">
      <ListboxButton class="relative w-full cursor-default rounded-md border border-gray-300 bg-white py-2 pl-3 pr-10 text-left shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm">
        <span class="flex items-center">
          <span :aria-label="selected.online ? 'Online' : 'Offline'" class="inline-block h-2 w-2 flex-shrink-0 rounded-full" :class="[selected.online ? 'bg-green-400' : 'bg-gray-200']" />
          ...
          <ListboxOption v-for="person in people" :key="person.id" v-slot="{ active, selected }" as="template" :value="person">
            <li class="relative cursor-default select-none py-2 pl-3 pr-9" :class="[active ? 'text-white bg-indigo-600' : 'text-gray-900']">
              <div class="flex items-center">
                <span class="inline-block h-2 w-2 flex-shrink-0 rounded-full" :class="[person.online ? 'bg-green-400' : 'bg-gray-200']" aria-hidden="true" />
