...
<template>
  <form action="#" class="relative">
    <div class="overflow-hidden rounded-lg border border-gray-300 shadow-sm focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500">
      ...
      <input id="title" type="text" name="title" class="block w-full border-0 pt-2.5 text-lg font-medium placeholder-gray-500 focus:ring-0" placeholder="Title">
      <label for="description" class="sr-only">Description</label>
      <textarea id="description" rows="2" name="description" class="block w-full resize-none border-0 py-0 placeholder-gray-500 focus:ring-0 sm:text-sm" placeholder="Write a description..." />
    ...
    <div class="absolute inset-x-px bottom-0">
      ...
      <div class="flex flex-nowrap justify-end space-x-2 py-2 px-2 sm:px-3">
        <Listbox v-model="assigned" as="div" class="flex-shrink-0">
          ...
          <div class="relative">
            <ListboxButton class="relative inline-flex items-center whitespace-nowrap rounded-full bg-gray-50 py-2 px-2 text-sm font-medium text-gray-500 hover:bg-gray-100 sm:px-3">
              ...
              <span class="hidden truncate sm:ml-2 sm:block" :class="[assigned.value === null ? '' : 'text-gray-900']">{{ assigned.value === null ? 'Assign' : assigned.name }}</span>
            </ListboxButton>
        ...
        <Listbox v-model="labelled" as="div" class="flex-shrink-0">
          ...
          <div class="relative">
            <ListboxButton class="relative inline-flex items-center whitespace-nowrap rounded-full bg-gray-50 py-2 px-2 text-sm font-medium text-gray-500 hover:bg-gray-100 sm:px-3">
              <TagIcon class="h-5 w-5 flex-shrink-0 sm:-ml-1" :class="[labelled.value === null ? 'text-gray-300' : 'text-gray-500']" aria-hidden="true" />
              <span class="hidden truncate sm:ml-2 sm:block" :class="[labelled.value === null ? '' : 'text-gray-900']">{{ labelled.value === null ? 'Label' : labelled.name }}</span>
            </ListboxButton>
        ...
        <Listbox v-model="dated" as="div" class="flex-shrink-0">
          ...
          <div class="relative">
            <ListboxButton class="relative inline-flex items-center whitespace-nowrap rounded-full bg-gray-50 py-2 px-2 text-sm font-medium text-gray-500 hover:bg-gray-100 sm:px-3">
              <CalendarIcon class="h-5 w-5 flex-shrink-0 sm:-ml-1" :class="[dated.value === null ? 'text-gray-300' : 'text-gray-500']" aria-hidden="true" />
              <span class="hidden truncate sm:ml-2 sm:block" :class="[dated.value === null ? '' : 'text-gray-900']">{{ dated.value === null ? 'Due date' : dated.name }}</span>
            </ListboxButton>
