...
<template>
  <div class="bg-white shadow sm:rounded-lg">
    <SwitchGroup as="div" class="px-4 py-5 sm:p-6">
      <SwitchLabel as="h3" class="text-lg font-medium leading-6 text-gray-900" passive>
      ...
      <div class="mt-2 sm:flex sm:items-start sm:justify-between">
        ...
        <div class="mt-5 sm:mt-0 sm:ml-6 sm:flex sm:flex-shrink-0 sm:items-center">
          <Switch v-model="enabled" class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2" :class="[enabled ? 'bg-indigo-600' : 'bg-gray-200']">
            <span aria-hidden="true" class="inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out" :class="[enabled ? 'translate-x-5' : 'translate-x-0']" />
          </Switch>
        </div>
      </div>
    </SwitchGroup>
