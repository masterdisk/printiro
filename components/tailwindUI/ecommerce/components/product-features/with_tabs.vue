            ...
            <div class="flex-auto border-b border-gray-200 px-4 sm:px-0">
              <TabList class="-mb-px flex space-x-10">
                <Tab v-for="tab in tabs" :key="tab.name" v-slot="{ selected }" as="template">
                  <button class="whitespace-nowrap border-b-2 py-6 text-sm font-medium" :class="[selected ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300']">
                    {{ tab.name }}
                  </button>
                </Tab>
              </TabList>
