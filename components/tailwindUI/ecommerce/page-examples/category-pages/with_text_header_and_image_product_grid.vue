                    ...
                    <div class="flex h-full justify-center space-x-8">
                      <Popover v-for="category in navigation.categories" :key="category.name" v-slot="{ open }" class="flex">
                        <div class="relative flex">
                          <PopoverButton class="relative flex items-center justify-center text-sm font-medium transition-colors duration-200 ease-out" :class="[open ? 'text-indigo-600' : 'text-gray-700 hover:text-gray-800']">
                            {{ category.name }}
                            <span class="absolute inset-x-0 -bottom-px z-30 h-0.5 transition duration-200 ease-out" :class="[open ? 'bg-indigo-600' : '']" aria-hidden="true" />
                          </PopoverButton>
                        </div>
          ...
          <div class="fixed inset-0 z-40 flex">
            <TransitionChild as="template" enter="transition ease-in-out duration-300 transform" enter-from="translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-300 transform" leave-from="translate-x-0" leave-to="translate-x-full">
              <DialogPanel class="relative ml-auto flex h-full w-full max-w-xs flex-col overflow-y-auto bg-white py-4 pb-6 shadow-xl">
                ...
                <!-- Filters -->
                <form class="mt-4">
                  <Disclosure v-for="section in filters" :key="section.name" v-slot="{ open }" as="div" class="border-t border-gray-200 px-4 py-6">
          ...
          <section aria-labelledby="filter-heading" class="border-t border-gray-200 pt-6">
            ...
            <div class="flex items-center justify-between">
                  ...
                  <MenuItems class="absolute left-0 z-10 mt-2 w-40 origin-top-left rounded-md bg-white shadow-2xl ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div class="py-1">
                      <MenuItem v-for="option in sortOptions" :key="option" v-slot="{ active }">
                        <a :href="option.href" class="block px-4 py-2 text-sm font-medium text-gray-900" :class="[active ? 'bg-gray-100' : '']">{{ option.name }}</a>
                      </MenuItem>
                    </div>
                  </MenuItems>
                </transition>
              ...
              <PopoverGroup class="hidden sm:flex sm:items-baseline sm:space-x-8">
                <Popover v-for="(section, sectionIdx) in filters" id="menu" :key="section.name" as="div" class="relative inline-block text-left">
                  <div>
